...
async function sendMessageToTelegramChat(token, chatId, message) {
...
export async function notifyTelegramChat(msg) {
    ...
    const chatId = process.env.TELEGRAM_CHAT_ID; // Use environment variable for chat ID

    if (token && chatId) {
        try {
            await sendMessageToTelegramChat(token, chatId, msg);
        } catch (e) {
            console.error('Error sending message to Telegram chat:', e);
        }
    } else {
...
const request = async (url, options) => {
  ...
  if (!response.ok) {
    ...
    await notifyTelegramChat(`[adblocker-cfgw-dns] An HTTP error has occurred (${response.status}) while making a web request. Please check the logs for further details.`);
